...
**用途**: 検出結果で`pivots_normalized`(標準ピボット表)を推測値から
実測値に置き換える。肘・膝の座標も取れるので、腕・脚を1枚のまま
(剛体2分割せず)メッシュ変形の曲げ点として使える。

**実行順序**: マスター(#1、`scripts/normalize_reference.js`で正規化済み)
に対して`joint_detection_master_v1.txt`を実行する。レイヤー分解(#2・#3)
の完了を待つ必要はない([`prompts/joint_detection_v1.txt`](./prompts/joint_detection_v1.txt)
はパーツ分解後の検証用に温存、両対応)。
