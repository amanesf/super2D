...
- [`reference/master_v1.png`](./reference/master_v1.png)(900×1600): ユーザー
  ...
  雰囲気の参照)**として毎回添付する。**ただしこの原画自体は標準ポーズ
  (全身直立)ではなく、片腕を曲げて発光する球体を持ち、ケープが片側に
  大きく流れる動きの大きいアクションポーズである**(1回目テスト後に
  判明した設計上の見落とし。次項参照)
...
1. **マスター(#1)確定**: `reference/master_v1.png`(元画像そのまま)は
   動きの大きいアクションポーズのため、そのままレイヤー分解に使わない。
   `prompts/front_view_v1.txt`で正面直立ポーズに整形し直した結果を
   新しい元画像として確定させる(1回目テストで判明した抜け漏れ)
2. **レイヤー分解(#2)を1シートだけ試す**(正面直立ポーズを元画像として
   使用し、`prompts/body_layers_v2.txt`で再試行) — 「1タイル1パーツを
   透過背景単独描画」「隠れた部分の自然な補完」が同時に成立するかを
   確認する。ここが崩れた場合(パーツが混ざる・透過が汚い・補完が破綻
   する等)は、その場でパーツ1つずつの個別呼び出しに切り替える(#3・#7も
   同様の構造なので同じ結論が転用できる)
...
生成)を独立した工程として挟む**([`prompts/front_view_v1.txt`]
(./prompts/front_view_v1.txt))。元画像を参照しつつ、腕を体から離した
自然な直立ポーズ・小物を持たない素手・ケープを左右対称に垂らした状態に
描き直させ、その結果を新しい元画像(`master_v2`相当)としてレイヤー
分解に使う。プロンプトの指示強化(v2)と、ポーズの正規化は**両方とも
必要**と判断し、両方を次の試行に反映する:

- [`prompts/front_view_v1.txt`](./prompts/front_view_v1.txt): 元画像から
  正面直立ポーズを生成する(新規追加、次に試すべき最初のステップ)
- [`prompts/body_layers_v2.txt`](./prompts/body_layers_v2.txt): グリッド線
  非表示の強調・ケープの相互排他明記・空白セルの明記・腕の境界明確化を
  反映したレイヤー分解プロンプト(正面直立ポーズの結果を元画像として使う)
