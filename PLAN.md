...
- **完了**: マスター(正面直立ポーズ)生成 → [`reference/master_v2.png`](./reference/master_v2.png)
...
- **未決着の論点**: 「360度なめらかな回転+アクション」の水準に届く
  ためには関節可動・アクションを手続き型(剛体回転、追加コストなし
  だがカクつく)に倒す案を提示したが、ユーザーからは却下ではなく
  「そういう意味ではない、試行錯誤を減らして確実に進めたい」という
  応答があり、コスト削減の方向では合意していない。次に決めるべき論点
  として残っている
- **今回の実現性検証全体で得た学び**: 旧new2D3D(SD1.5世代)の検証結果は
  前提として持ち込まない/切り分けもGeminiに行わせる/新しいプロンプトは
  支払って試す前に矛盾がないか机上レビューしてから渡す、の3点
