...
6. **識別要素をテキストにも毎回明記する**: 髪色・瞳の色・ヘッドセットの
   色/形・衣装配色をプロンプト本文に書く(画像添付だけに頼らない)
7. **雰囲気(画風)の維持をテキストで指示する**: 線の質・塗り方・配色の
   トーンといった画風は言葉で書き下す必要はなく、「1枚目(元画像)の
   雰囲気を維持すること」と一文で指示すれば足りる。背景色(キー抜き用の
   単色指定)だけは機能的に必要なため文章で別途明記する
8. **変更してよい範囲/してはいけない範囲を分離する**: 「表情(目・口)
