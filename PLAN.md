...
- **確定した設計方針(最新)**: Gemini API利用前提(従量課金は総額数ドル
  〜数十ドル程度と判明し、過度なコスト回避は不要と判断)。**正面±30度
  (0°・30°・330°)のみ表情・viseme・手の形・関節・アクションをフル
  充実させ、60°〜300°は角度ごとの全身スプライトのみで表情・アクションは
  対象外**。設計は**モデル非依存(キャンバス規格・前処理・命名規則・
  マニフェスト)とモデル依存(プロンプト文面)を明確に分離**し、入力
  画像は[`scripts/normalize_reference.py`](./scripts/normalize_reference.py)
  で毎回同じ規則(900×1600キャンバス、キャラクター高さ1400pxに統一)で
  前処理する
...
## 設計の分離: モデル非依存 vs モデル依存(2026-07-21追加)

「モデルによらず標準化・構造化・再現性に重点を置く」という方針を受け、
設計要素を2種類に明示的に分ける:

- **モデル非依存(将来Geminiを他モデルに差し替えても変わらない)**:
  標準キャンバス規格(900×1600)・入力画像の前処理ルール・パーツ命名
  規則・マニフェストスキーマ・標準ピボット表・角度の刻み(30度)・
  パーツ分解の境界(切ってよい場所の判断基準)・組み立て方(クロス
  フェード/メッシュ変形の使い分け)
- **モデル依存(Gemini固有、差し替え可能)**: 実際のプロンプト文面
  (【役割】【カメラ】等の中身)・API呼び出し方・「元画像1枚 vs
  テンプレ画像併用」のような入力形式の具体的な運用

この文書内でも、以後どちらに属する話かを意識して書き分ける。モデル
依存の部分(プロンプト文面)は`prompts/`配下のテキストファイルとして
分離済みなので、モデルを変える場合はそこだけ差し替えればよい設計に
なっている。
...
[`scripts/normalize_reference.py`](./scripts/normalize_reference.py)

**規則**:
- 出力キャンバス: 900×1600、白背景(標準テンプレートと同一)
- 入力画像の四隅の色を背景色として自動検出し、背景と異なる領域を
  キャラクター本体とみなしてバウンディングボックスを検出する
- バウンディングボックスの高さが**1400px(キャンバス高さの87.5%)**に
...
正規化済み(バウンディングボックス849×1400、オフセット(25, 100))。
...
**正面±30度(0°・30°・330°)だけフル充実、それ以外は基本スプライトのみ
という設計を確定**(2026-07-21、ユーザー決定):

- **0°・30°・330°(正面から30度以内)**: レイヤー分解(#2・#3)・表情
  (#5)・viseme(#6)・手の形(#7)・関節可動(#8・#9)・アクション
  (#10)を**フルセットで用意する**。VTuberとして最も見られる範囲であり、
  ここの表現力がLive2D比較の勝敗を分けるため、コストを惜しまず充実させる
- **60°〜300°(9方向)**: レイヤー分解・表情・アクションは持たせず、
  **角度ごとの全身1枚のスプライトのみ**とする。表情・アクションは今回の
  スコープ外と確定し、「必要になったら追加」という保留ではなく明確に
  対象外とする。背面まで表情豊かに動かす必要性は薄いという判断
- この切り分けにより、#5〜#10(表情/viseme/手の形/関節/アクション)の
  生成対象は実質「0°・30°・330°の3方向」に限定される。ただし現状の
  各シートは1回の呼び出しで全カテゴリを賄う設計なので、3方向分を
  別々に生成するかは今後の検討課題(まずは0°=マスターで1セット作り、
  30°・330°で表情差分等が要るかは実際の見た目を見て判断する)
...
1. **マスター(#1)— 完了**: `reference/master_v2.png`(正面直立ポーズ、
   `scripts/normalize_reference.py`で正規化済み)
...
6. 新しく生成した参照画像・派生アセットは`scripts/normalize_reference.py`
   を必ず通してから次の入力として使う
