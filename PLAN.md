...
### Stage B — ビューア: 読み込み/書き出し UIとカタログ全状態への対応

- 現行はfetch先が`assets/manifest.json`に決め打ち。`characters/<id>/`を
  選べるように一般化する(まずはURLクエリ`?character=zero`程度の簡易
  切替でよい。複数キャラのGUIセレクタは後回しでよい)
...
### Stage D — 画像作成側のツール(Gemini出力の取り込み、プレースホルダーで検証可能)

PLAN.mdの「次のアクション」#5で「結果が見えた時点で新規実装する」と
していた組み立てスクリプトがこれ。実際のGemini生成を待たずに、**偽の
シート画像+偽の検出JSON**を使って先に作り検証する:

- `scripts/ingest_sheet.js`: シート画像(グリッド)+セル配置定義+
  セル→パーツ/状態名の対応表を渡すと、個別パーツPNGに切り出して
  `characters/<id>/source/`に書き込む(決定的処理、AI不使用)
- `scripts/apply_joint_detection.js`: `joint_detection_v1.txt`形式の
  検出結果JSON(パーツごとのローカル座標)を受け取り、`character.json`
  の該当パーツの`pivot`/`anchors`を更新する
- 上記2つを、このリポジトリ内で自作した偽シート+偽JSONに対して
  実際に動かして検証してからStage Fに進む

### Stage E — 圧縮ビルドステップ

- `scripts/build_public_assets.js`: `characters/<id>/source/*.png`から
  `characters/<id>/public/*.webp`を生成し、`character.json`の実行時
  `src`を`public/`側に向ける
- **要検証**: このサンドボックス環境でWebPエンコードができるパッケージ
  (`sharp`等)が実際に導入できるかは未確認(ネットワーク・ビルド環境
  次第)。Stage Eの最初の小タスクとして着手時に確認し、導入できなければ
  PNG再圧縮(解像度を落とす・パレット化)にフォールバックする方針で
  進める

### Stage F — 本番画像生成(最後)

Stage A〜Eをプレースホルダーで固めてから着手する。`prompts/`配下に
既に用意済みのプロンプトを実際にGeminiへ投げ、その結果をStage D の
取り込みツール・Stage E の圧縮ビルドに通して`characters/zero/`(本番
キャラ)を作り、プレースホルダーと差し替える。
