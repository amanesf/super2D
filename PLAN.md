...
**悲観シナリオ(#2・#3・#7・#8・#9・#11のまとめ込みが崩れ、パーツ/状態
ごとに個別呼び出しが必要になった場合)**: #2(7パーツ個別)+#3(5パーツ
個別)+#7(8状態個別)+#11(3状態×4角度=12個別)を分解すると、マスター1+
検出1+分解体7+分解顔5+角度12+表情core1+表情拡張1+viseme1+手8+
アクションcore1+アクション拡張1+ロコモーション12 = **約51回**。さらに
生成は確率的なので、一定割合は再ロールが必要になる前提を置くと、実運用
では**60回前後**まで見ておくのが安全。
...
(+レイヤー分解(顔)側の前髪・頭部・目・口)

**検出結果に応じた可変パーツ**:
| 検出結果 | 対応 |
|---|---|
| `hair_back_type: "twin"` | `hair_tail_l` + `hair_tail_r`の2セル(1セルの`hair_back`の代わり) |
| `hair_back_type: "single"` | `hair_back`1セル(このキャラの現行設計のまま) |
...
| `misc_accessories` | **個別にセルを割かず、まとめて1セル`accessories`に集約**(低頻度のため。将来的にどれか特定の小物が重要になったら専用セルに格上げする) |

8セル(2列×4行)に収まらない場合は、体パーツ用シートと可変パーツ
(アクセサリー)用シートの2枚に分ける。**このキャラ(zero相当)の場合**:
`hair_back_type: "single"`・`flowing_cloth: ["cape"]`・他は該当なしと
想定され、胴/右腕/左腕/右脚/左脚/後ろ髪/ケープの7セルで従来設計と一致
する(1シートに収まる)。
...
```json
{
  ...
  "detected_parts": {
    "_comment": "prompts/part_detection_v1.txt の出力をそのまま保存(次のキャラで再実行した際の差分確認・デバッグ用)",
  ...
  "parts": {
    ...
    "accessories": {
      ...
      "items": [],
      "note": "misc_accessoriesが1件以上あればここに1パーツずつ入れる。このキャラは空"
    },
  ...
  "draw_order": ["body_angle", "torso", "hair_back", "cape", "accessories", "arm_l", "arm_r", "face", "hand_l", "hand_r"]
}
```
...
表す。固定7パーツ前提だった旧設計から、**パーツ構成そのものがキャラ
