...
- **確定した設計方針(最新)**: Gemini API利用前提(従量課金は総額数ドル
  ...
  [`scripts/normalize_reference.py`](./scripts/normalize_reference.py)で
...
- [`reference/master_v2.png`](./reference/master_v2.png)(1024×1024):
  ...
  `scripts/normalize_reference.py`で1024×1024に正規化したもの。
...
| 10 | アクションポーズシート | 挨拶(手を振る)/お辞儀/頷き/待機(正面±30度のみ) | 1シートにタイル化(実証済み手法と同型) | 1 |
| 11 | ロコモーションシート | 歩行中間/走行中間/跳躍。**角度を問わず必要**なため主要4方向(0°・90°・180°・270°)分を用意 | 角度ごとに1シート(3状態をタイル化) | 4 |
| | **合計(楽観)** | | | **約25回** |
...
個別)+#7(4状態個別)+#8(4状態個別)+#9(3状態個別)+#11(3状態×4角度=
12個別)を分解すると、マスター1+分解体7+分解顔5+角度12(変わらず)+
表情1+viseme1+手4+関節肩肘4+関節股膝3+アクション1+ロコモーション12 =
**約51回**。さらに生成は確率的なので、一定割合は再ロールが必要になる
前提を置くと、実運用では**60回前後**まで見ておくのが安全。
...
ための設計。この形式は確定ではなく、実装着手時に見直す前提のたたき台。
