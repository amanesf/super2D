...
**既知の未修正課題(2026-07-24、ユーザー確認済み・優先度は保留)**:
サイド髪(hair_side_l/r)は前髪と違って土台と同じ共有メッシュのまま
実装しているため、揺れ幅を上げた際に隣接する腕の領域まで歪む現象が
確認されている(前髪で発生したのと同じ「独立して動く部位はz順不変でも
分離が必要」の原因)。今回は保留とし、いずれサイド髪も前髪と同様に
独立レイヤー化するか、腕側にも顔と同じ「絶対に動かさない」マスクを
適用する対応が必要。
