...
[`prompts/angle_turntable_v1.txt`](./prompts/angle_turntable_v1.txt)の
`{ANGLE}`を各角度の数値に差し替えて12回individually呼び出す。角度間で
前の生成結果をつなぐ(img2imgチェーン)のではなく、**毎回同じマスター
画像を基準にする**(旧SD1.5系検証⑫⑬のimg2imgチェーンは連続性と引き
換えに回転量が乗らない問題があったため、そのパターンは踏襲しない)。
