...
5. **元画像を毎回添付し、雰囲気(画風)の維持をテキストで指示する**:
   線の質・塗り方・配色のトーンといった画風は言葉で書き下す必要はなく、
   マスター画像(または直近の生成物)を毎回添付した上で「添付画像の
   雰囲気を維持すること」と一文で指示すれば足りる。画風の記述をこちらで
   固定するのではなく、Geminiが参照画像を見て合わせる前提に立つ。
   背景色(キー抜き用の単色指定)だけは機能的に必要なため文章で別途
   明記する
