...
### 関節点検出(Phase 0.5、ピボット実測とメッシュ曲げ点の指定、2026-07-21追加)

「動きの質」レビュー(2026-07-21)で判明した最大のギャップは、
`pivots_normalized`の座標がキャリブレーションなしの推測値のままだった
こと。**ここもGeminiに検出させる**(ユーザー提案: 「肘関節をここに
置いてみたい」を人手ではなくAIに指示させる)。

**この工程は`ghostitd`のランドマーク推定と問題の形が一致する**(座標を
画像から検出するタスク)ため、`GHOST_SCANNER_PLAN.md`で実証済みだった
テクニックをそのまま使う([`prompts/joint_detection_v1.txt`](./prompts/joint_detection_v1.txt)
に反映済み):
- キャリブレーション手順(座標を打つ前にパーツの輪郭範囲を先に把握)
- 画面向かって右/左を明記(「本人視点」表記は左右反転の実害があった)
- セルフチェック手順をプロンプト末尾に入れる

**用途**:
1. `pivots_normalized`(標準ピボット表)を推測値から実測値に置き換える
2. **肘・膝を関節点として検出することで、腕・脚を1枚のまま(剛体2分割
   せず)メッシュ変形の曲げ点として使える**。前回レビューの「メッシュ
   変形の具体仕様が無い」の一部を解消(曲げ点の座標は決まるが、頂点
   密度・分割方法自体はまだ別途必要)

**実行順序**: レイヤー分解(#2・#3)が生成できてから、その結果画像に
対してこの検出を実行する(生成前には検出できない)。マスター(#1)への
検出ではなく、実際に切り分けられたパーツ画像に対して行う。
