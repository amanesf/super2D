...
- バウンディングボックスの高さが**896px(キャンバス高さの87.5%)**に
  なるよう等比拡大縮小し、水平中央揃え・垂直方向は下端24px固定余白の
  非対称配置(足元は地面に接するため余白不要、余った縦余白は全て
  上に回して髪の揺れ・持ち物等の可動余地にする。2026-07-23、実イラスト
  検証でユーザー指摘により垂直中央揃えから変更)
...
### 抜け漏れの訂正: 関節メッシュ変形自体はC2の範囲外だった(2026-07-22)

C2実装時、`js/viewer.js`をWebGL描画へポートしたが、`arm_lower_r/l`・
`leg_lower_r/l`(`motion: "procedural-mesh-bend"`)は**従来通り剛体2分割の
回転のまま**とし、C1で実証した頂点スキニングによる関節メッシュ変形は
意図的にスコープ外にした(C2はレンダラの土台・レイヤー合成・固定
タイムステップが対象で、関節の継ぎ目解消そのものは含まない判断)。

その結果、**C1で確認した「肘を大きく曲げると継ぎ目が出る」問題は、
現時点のリグにはまだ反映されていない**。実画像を差し替えてもこの問題は
再現する。TASKS.mdにタスクC2.5として追加し、Stage C3〜C5より先に着手する
位置に置いた。

## D0a机上レビュー: `prompts/part_insitu_v1.txt`(2026-07-22)

支払う前にプロンプトを詰める慣習(本文中節参照)に従い、in-situパーツ
生成テンプレートの初稿を自己レビューした。

**指摘1(規約からの逸脱・実害リスク)**: 初稿はテンプレート本文の後ろに
「プレースホルダー一覧」という説明セクションを`---`区切りで付けていたが、
既存のプロンプト群(`angle_turntable_v1/v2.txt`等)は`{ANGLE}`のような
プレースホルダーをテキスト中にインラインで置くだけで、意味の説明は
プロンプトファイル自体には書かない(実行時に置換関数・パイプライン
定義側が持つ、というCLAUDE.mdの規約)。説明セクションを残すと、素朴な
実装(ファイルをそのままfetchしてAPIに渡す)でAPIに送られてしまう実害
リスクがある。**修正**: 説明セクションを削除し、他ファイルと同じ
インラインプレースホルダーのみの形にした
- **`PART_DESC`にキャラクター固有名詞(髪色・目の色・キャラ名等)を
  含めない**という注意点は、削除した説明セクションに書いていた内容の
  うち唯一「実装時に忘れると事故る」重要な情報だったため、本書
  (PLAN.md)に移して記録する。D1でパーツ表を実装する際に反映すること
- **指摘2(冗長・トークンコスト)**: 【役割】と【カメラ・構図】の2節が
  「マスターと同じ位置・スケールを保つ」という同じ内容を重複して書いて
  いた。50〜60回の実API呼び出しでコストが積み重なるため、【カメラ・
  構図】節を廃止し内容を【役割】に統合して簡潔にした

**機械生成の検証**: `torso`/`arm_upper_r`/`arm_lower_r`/`hair_back`の
4パーツ×白背景/黒背景の計8バリエーションで置換関数を通し、未置換の
プレースホルダーが残らないこと・出力文面が矛盾なく読めることを確認した
(検証コードはプロダクト外、スクラッチ領域で実行し破棄)。

## Stage G(将来構想): 音声対応・リアルタイム会話対応(2026-07-22追加、未着手)

ユーザー指示により将来計画に追加。**現時点では詳細設計・実装ともに
未着手**。Stage A〜Fの完了後、着手前に改めて詳細設計を行う。

### 狙い

- **表情・アクションと連動する音声対応**: キャラクターがしゃべる際、
  発話内容に応じて音声(TTS)を再生しつつ、口パク(既存の
  `mouth`のviseme状態: `rest`/`aa`/`ih`/`ou`/`ee`/`oh`、C3で
  クロスフェード実装済み)と表情(`head_base`の12状態)・
  アクション(`body_pose`等)を連動させる
- **リアルタイム会話対応**: ユーザーの入力(テキスト、将来的には音声も
  想定)に対してキャラクターがその場で応答し、上記の音声・表情・
  アクション連動を伴って会話が成立する体験

### 現時点で見えている論点(決定事項ではない、着手時に検討)

- **アーキテクチャ上の大きな変更点**: 現行設計は「Gemini呼び出しは
  `studio.html`でのアセット作成時のみ、`viewer.html`での通常再生・
  閲覧はAPI呼び出し無しで完結する」という前提(CLAUDE.mdの「ビルド無し
  ・サーバー無し」方針とも整合)。リアルタイム会話は**閲覧時にも
  API呼び出しが発生する**ことを意味し、この前提を変える。APIキー管理
  (`js/gemini_api.js`)を`viewer.html`側にも持たせる設計変更が必要になる
- **音声合成の方式**: ブラウザ標準のWeb Speech API(`SpeechSynthesis`、
  無料・サーバー不要・音声の表現力は限定的)か、Gemini等のAPIベースの
  音声合成(表現力は高いが呼び出しごとに課金・レイテンシが発生)かは
  未決定。「サーバー無し」方針とAPIコストの両面から検討が要る
- **口パクのタイミング同期**: 現行の`updateTalk()`は`sin(t*14)`による
  機械的なon/off切替(見た目のデモ用)。実際の発話に同期させるには、
  音声合成側から音素(phoneme)ごとのタイムスタンプを取得できるか、
  もしくは音量(振幅)ベースの簡易口パク推定(音素までは同期しない)で
  妥協するかの選択が要る
- **表情選択のロジック**: 会話の応答内容に応じてどの表情
  (`head_base`の12状態)を選ぶかは、応答生成側(LLM)にタグ付けさせる
  (例: 応答JSONに`emotion: "happy"`を含めさせる)か、応答テキストを
  別途感情分類にかけるかの選択が要る
- **既存資産の再利用**: 口パク(C3クロスフェード)・表情切替(B3状態UI)
  ・呼吸/揺れ等のアイドルアニメーション(C2系)は既存のまま流用できる
  見込みが高い。新規に必要なのは「音声再生」「発話内容から状態を
  決定するロジック」「(リアルタイム会話の場合)ユーザー入力の受付と
  応答生成」の3点

### 着手条件

Stage A〜F(F1のランブックによる実生成完走)が完了し、キャラクター
バンドル自体が安定してから着手する。現時点でTASKS.mdへの具体タスク
分割は行わない(設計が固まっていない段階でタスク化すると手戻りが
大きいため)。着手時に本節を土台として詳細設計を行い、TASKS.mdに
Stage Gとしてタスクを追加する。

## 切り抜き方式への転換とアセット設計(2026-07-24、仮案・確定ではない)

**この節は「①決定論: シート方式を廃止、1パーツ=1呼び出し・in-situ
生成へ」(2026-07-22)を、正面フルリグの部分に限り上書きする。** 会話しながら
1カテゴリずつ確定させていく暫定案であり、後続の会話で内容が変わりうる。

### 転換の理由

in-situ生成(パーツごとにGeminiへ再描画させる方式)を実際に運用した結果、
生成→合成の過程で不要な縮小(S≈0.54〜0.73倍)がかかり、マスター原画に
対して明確に解像度が劣化する実装バグが見つかった。加えてGeminiの再描画は
腕・脚の誤生成(肌の塊化・サイズ異常)を伴い、QCで弾いても再ロールの
不確実性が残る。実在のLive2Dモデル制作(1枚絵を作者が手動でレイヤー分解・
オクルージョン部分だけ手描き補完し、そこから先はメッシュ変形で動かす)を
参考に、**「マスターの実画素をそのまま切り出す」方式に転換**した。

### 新方式(マスター+色分けラベルマップ+ローカル切り出し)

1. **マスター(Tポーズ)**: `prompts/tpose_from_master_v1.txt`。短く直接的な
   プロンプト+セルフチェック節(左右非対称要素の誤複製・パーツ増減・
   両腕水平・デザイン同一性を出力前に自己確認させる)。1回で成功実績あり
2. **色分けラベルマップ**: `prompts/segmentation_v1.txt`。マスターと同一
   構図で、パーツごとに指定色で塗り分けた1枚を1回の呼び出しで生成する。
   衝突なしパレット(全ペアRGB距離85以上、色相環等間隔配置)+セルフ
   チェック節(塗り漏れ・未使用色・左右取り違え・非対称要素の誤複製を
   出力前に自己確認)を組み込み済み
3. **ローカル切り出し(Gemini呼び出しゼロ)**: ラベルマップの各画素を
   最寄りパレット色で分類し、対応するマスター実画素をパーツごとに
   切り出す。連結成分分析で最大成分の5%未満の微小成分(JPEG境界ノイズ)
   のみ除去し、同一部位が前髪などで分断された複数の島は正しく採用する
   (これを誤って「最大の連結成分のみ採用」にすると顔が欠ける、という
   失敗を最小実証で確認済み)
4. **二段検証**(呈示前に両方パス必須): 数値的な不備(色衝突・塗り漏れ・
   未分類画素)は`verify_segmentation.js`でローカル機械判定。意味的な
   不備(左右取り違え・非対称要素の複製・形状のずれ)は
   `prompts/segmentation_review_v1.txt`でGeminiに再度判定させる
   (テキストのみ、軽量)

**最小実証(2026-07-23)**: torso+head_baseの2パーツで上記手順を実行し、
実画素・無劣化のパーツ抽出とビューア上での実際のアニメーション(呼吸・
sway、フレーム差分で機械確認済み)を確認した。詳細は
`characters/cutout-test/`。

### 「Live2D並+360度回転」の技術的な位置づけ

真の連続360度回転は2Dレイヤー合成の原理的限界(1枚の正面絵をメッシュ
変形できるのはせいぜい±30〜40度、輪郭裏側の画素がそもそも存在しない)
であり、**実在のLive2Dモデル自体も行っていない**。要件は
**離散方向マスター(8〜12方向)+各方向内はLive2D品質のメッシュ変形/
パララックスでリグ、方向をまたぐ時はスプライト切替**という二層構造で
満たす(ユーザー確認済み、2026-07-24)。真の連続3D回転は対象外。

### 全体アセット設計(仮案、呼び出し数は目安)

| # | カテゴリ | 内容 | 方式 | 呼び出し目安 |
|---|---|---|---|---|
| 1 | 正面マスター | 全身1枚 | 単独 | 済(1) |
| 2 | パーツ検出 | 可変パーツ有無(JSON) | テキストのみ | 1 |
| 3 | 正面 色分けラベルマップ | torso/head_base/hair×4/cape/腕脚等 | 単独、セルフチェック込み | 1(+予備1) |
| 4 | 正面 ラベルマップ意味レビュー | 左右取り違え等の検出 | テキストのみ | 1 |
| 5 | 正面 まばたき半目+閉眼 | 目のみの小領域 | 単独 | 1(+予備1) |
| 6 | 正面 viseme5状態(aa/ih/ou/ee/oh) | 口のみの小領域、まとめて1枚 | 単独 | 1(+予備1) |
| 7 | 正面 表情差分(core6+拡張6) | 顔まわり12状態 | 2Kシート(要検証) | 1〜2(+予備1) |
| 8 | 回転 他方向マスター(11方向) | 全身×11 | 2K・2×2グリッド4枚/回(要検証) | 3(+予備1) |
| 9 | 各方向の背景抜き | 平面スプライト化 | ローカル処理(matting.js) | 0 |
| 10 | 正面 手の形(core4+拡張4) | 右手8状態 | 2Kシート | 1(+予備1) |
| 11 | アクション用キー差分 | 内容は会話しながら決める(未定) | 2Kシート(想定) | 未定 |
| 12 | ロコモーション(歩行/走行/ジャンプ×主要4方向) | 平面スプライト | 2Kシート | 3(+予備1) |

呼び出し数目標の20回上限は撤回した(要件を完全に満たすことを優先する。
ユーザー指示、2026-07-24)。ただし無駄な呼び出しは引き続き避ける
(セルフチェック・機械検証・意味レビューで防げるものは事前に潰す)。

### 未検証事項と進め方

- **2Kグリッド詰め込み(#7・#8・#10・#12)は未検証**。まず#8(回転、
  最もコスト効果が大きい)で2方向のみの小さい実験を行い、グリッド線
  写り込み・セルはみ出しが起きないかを確認してから他カテゴリに展開する
- 上記アセット表は暫定案。**1カテゴリずつ会話で確定させながら進める**
  (ユーザー指示、2026-07-24)。#11(アクション)は具体的な種類も未定

### ユーザーの設計方針(記録、2026-07-24)

色分け方式(切り出し方式)そのものへの疑問を受けた議論で明確になった、
今後の設計判断すべてに適用すべき方針。**私(実装側)が新しい案を作る際は、
まずこの方針に照らして評価すること**。

- **再現性・効率性・組み立ての容易さを最優先する**。「動くから良い」
  ではなく、キャラが変わっても・時間が経っても・作り直しても同じ手順で
  同じ品質が出ることを重視する
- **「可動部位を減らす」とはスコープ(動かす種類)を削ることではなく、
  仕組み(実現方法)をシンプルにすること**。動きの豊富さは落とさず、
  それを実現するための部品点数・工程数・Gemini呼び出し数を減らす方向で
  考える
- **極力1枚絵に近い状態を保つ**という発想を持つ。パーツを物理的に
  切り分けて別ファイルにする(継ぎ目・pivot・z順の管理が発生する)より、
  1枚のテクスチャのままWebGL側(頂点スキニング等)で可動域を割り当てる
  方法を優先的に検討する。Gemini(画像でもテキストでも)は「どこにどう
  重みを割り当てるか」を判断させる用途で使い、「画素を切り分けて別画像を
  作らせる」用途は本当に必要な場合(前後関係=z順が変わる部位)に限定する
- この方針の具体化として、2026-07-24時点で「スキンウェイト方式」
  (色分けラベルマップを画像の切り出し型紙としてではなく、WebGL
  スキニングの重み境界を決めるガイドとして使う案)を検討中。評価待ち、
  未決定
- 正面±30度の関節メッシュ変形(頂点スキニング)がC2.5として未実装
  のまま残っている(最優先修正、既存タスク)。呼吸`scaleY`が首アンカーの
  変換に反映されず継ぎ目が開閉するバグも未修正

### 動き方式の比較検討(2026-07-24、他方式の網羅確認)

「本当にLive2D並にヌルヌル動くか」の確認要求を受け、2Dキャラアニメの
主要な実現方式を比較した。

| 方式 | 仕組み | ヌルヌル度 | 採否 |
|---|---|---|---|
| A. 剛体パーツ回転 | 関節中心にパーツ画像を剛体回転 | ✗ | 不採用(C1実験で60度から継ぎ目破綻を実測済み) |
| B. 骨格スキニング(Spine/DragonBones方式) | 少数〜1枚のテクスチャに頂点ごとの複数ボーン重みを持たせ連続変形 | ◎ | z順が変わらない部位で採用検討中 |
| C. FFD/デフォーマ方式(Live2D本家の方式) | パーツごとに切り分けた素材をワープ/回転デフォーマで連続変形。前後関係は描画順の明示的切替で対応 | ◎ | z順が変わる部位(腕の交差等)で採用 |
| D. ブレンドシェイプ/クロスフェード | 同位置の複数テクスチャを透明度で切替 | ○(テクスチャ差分のみ) | 表情・まばたき・口の形で既に採用(PLAN②節) |
| E. 物理レイヤー(バネ/Verlet) | 上記いずれかの上に遅延・慣性を追加する二次揺れ | ◎ | 髪・布の揺れで採用(B/Cと併用が前提、単独では成立しない) |
| F. 生成AIによるフレーム単位動画生成 | 動き自体をAIに都度生成させる | 未知数 | **却下**。「viewerはAPI呼び出し無しで完結」「ビルド無し・サーバー無し」方針に反し、再現性(ユーザーが最重視)とも相容れない |

**結論**: 本家Live2D自体はB(共有テクスチャ)ではなくC(切り分け+
デフォーマ)を採用しているが、ヌルヌルさの本質は「剛体回転(A)ではなく
連続的なメッシュ変形を使うこと」であり、B・Cのどちらでも同水準の
滑らかさになる(WebGL頂点スキニング、C1で確定済みの技術で両方実現
できる)。違いは継ぎ目の見え方のみ:
**z順が変わらない部位(呼吸・頷き・房内の揺れ等)はB(共有テクスチャ+
スキニング)を優先し、z順が変わる部位(腕の交差・肩越しに揺れる髪等)
のみC(切り分け)を使うハイブリッドを採用方針とする**。本家が全パーツを
切り分けているのは前後関係管理のためであり、切り分け自体が滑らかさの
必要条件ではない。

**訂正(2026-07-24、髪ゆれサンプルの実装で判明)**: 上記の「z順が
変わらなければB(共有テクスチャ)でよい」という基準は不十分だった。
前髪はhead_baseの手前に常に固定されており(z順は変わらない)、B方式で
実装したところ、前髪が動くたびに同じメッシュを共有するhead_base(顔)側の
三角形も引きずられて歪む(顔がグニャグニャする)問題が発生した。
**正しい基準は「独立して動く(動かない部位と隣接しつつ別々に動く)なら、
z順が変わらなくてもレイヤーを分離する必要がある」**。B方式が安全に
使えるのは、隣接領域同士が滑らかに連続変形する場合(呼吸で胴体が伸縮
しても首から下は同じ動きに追従する、など)に限られる。「顔の上を
独立に動く前髪」のような、静止領域と重なりながら別々に動く関係は、
たとえ前後関係が固定でもCの一種(レイヤー分離)が必要。
`demos/hair_sway/`で実装した具体形: 土台レイヤー(顔・胴・z順不変で
連動する部位、完全静止 or 一体で連続変形)+前髪オーバーレイ
(透明背景・独立スキニング)の2レイヤー合成。土台側は前髪が隠していた
額の部分を拡散法(ぼかしを繰り返して周囲の色を内側に染み込ませる、
Push-Pull系の簡易inpaint)で穴埋めし、オーバーレイがどれだけ動いても
土台に穴や二重描画が見えないようにした。

**解決済み(2026-07-24追記)**: 上記のサイド髪問題は、振れ幅を上げた
状態で首・胴体(顎・襟)まで歪む形で再発した(ユーザー指摘「輪郭歪む」)。
gravityGainを一時的に0にする切り分けテストでサイド髪が原因と実測確認し、
前髪と同じ独立オーバーレイ方式に統一して解消した。土台(base)は
無加工のマスターのままとし、前髪・サイド髪(左右)をそれぞれ独立した
透明背景レイヤーとして重ねる構成にした(`activeChains`をレイヤーごとに
分離)。

### 前髪・サイド髪(もみあげ)の設計(2026-07-24、確定)

顔まわりで最も目立つ部位として個別に検討した。マスター
(`master_tpose_1024.png`)を拡大確認した結果、このキャラには前髪
(`hair_front`、中央分けで左右に流れる1塊)とサイド髪
(`hair_side_l`/`hair_side_r`、頬〜肩まで届く1房ずつ)のみがあり、
独立した「もみあげ」の房は別途存在しない(hair_side_l/rが実質その役割を
兼ねる)。

**方式**: B(共有テクスチャ+スキニング)を採用。マスター画像はハサミで
切り取らず、透明な網目(メッシュ)を重ねて歪ませる方式(ゴムシートの
たとえ)。画素を切り出さないので新規描画は不要、色分けラベルマップは
「切り出しの型紙」ではなく「網目をどこに設定するかのガイド」として
引き続き1回必要。

**オクルージョン対策(構造的解決)**: 房を**根元(生え際)を固定した物理
チェーン**として実装する。根元の頂点は常にマスターと同じ位置に固定し、
毛先に近づくほど揺れを大きくする。根元が動かない限り、head_baseの
「前髪の下に隠れていて画素が存在しない領域」は原理的に露出しない
(振幅の数値によらず安全)。
...
| 房 | セグメント数 | 剛性 | 理由 |
|---|---|---|---|
| hair_front(前髪、中央で左右2チェーンに分割) | 1〜2 | 硬め | 短く額に近い、大きく揺れると不自然 |
| hair_side_l / hair_side_r | 2〜3 | 中程度 | 肩まで届く長さ |
| hair_back(後ろ髪、長いポニーテール状) | 3〜4 | 柔らかめ | 最も長く先端がしなる |

左右・前髪の左右チェーンは必ず位相・周波数をずらす(完全対称の揺れは
機械的に見える、PLAN②節の既定方針)。

**画像追加が必要なもの**: 髪自体は無し(マスター画素の変形のみ)。
まばたき(閉眼・半目)・viseme等、マスターに存在しない絵は引き続き
別途小さく生成する(髪の話とは独立)。

### `scripts/gemini_call.js`(2026-07-24追加): Gemini生成物の生データ保存専用ツール

過去に生成パイプライン内でリサイズ後の画素だけを保存し元解像度データを
永久に失ったバグ(2026-07-23、in-situ生成時)の再発防止として、
「Geminiへの呼び出し+応答の生データ保存」だけを行う専用ツールを作った。
**リサイズ・再圧縮・切り抜きのコードパスを一切持たない**設計にすることで、
バグを構造的に再発不能にしている。応答画像バイト列・応答JSON全体・
来歴ログ(`generation_log.jsonl`: 時刻・モデル・プロンプトファイル・
入力画像ハッシュ・imageSize・トークン数・保存先)を毎回セットで残す。
後続の加工(切り出し等)は、保存された生ファイルの**コピー**に対して
別スクリプトで行う運用とする。開発補助専用(CLAUDE.md「開発セッションから
実APIを呼ぶことは想定しない」の例外として、ユーザーの明示許可のもとで
検証目的に使う。APIキーは環境変数`GEMINI_KEY`から読み、リポジトリには
持ち込まない)。

## メッシュ変形方式の断念とフレームアニメ方式への全面転換(2026-07-24)

**この節は、これまでのC1(WebGLメッシュ変形リグ)・全身360度回転・
ロコモーションを含む設計のほぼすべてを上書きする、本日時点の最新方針。**
矛盾する記述が過去節にあれば、常にこの節が勝つ。

### 転換の理由

`demos/hair_sway`の物理メッシュ変形(バネによる頂点変位)の調整で
15コミット以上を費やしても、ゴースト・輪郭歪み・首の伸びといった
破綻が収束しなかった。原因を分析した結果、以下が判明した:

- Claude(実装側)は生成された絵を見て「歪んでいるか」「自然に動いて
  いるか」を正確に判断する能力が無く、パラメータ調整が当てずっぽうに
  なる
- 「元絵の維持度」を数値指標化しても(面積比の歪み率など)、歪みゼロを
  達成しようとすると可動域がほぼゼロに収束し、動かせなくなるという
  別の矛盾に突き当たる(ユーザー指摘、2026-07-24)
- 剛体回転・スキンウェイト境界の自動導出など複数の代替案も、いずれも
  「関節の継ぎ目」という同じ構造的問題に回帰した

一方、本日の実機テストで**Geminiは「怒り」「驚き」のような意味・概念を
伴う指示には高精度で応答し(表情12種グリッド、一発で高い一貫性を確認)、
「30度回転」のような数値・幾何指定には低精度だった**(回転グリッド実験、
30度・60度指定がほぼ正面のまま区別つかず、120度指定が180度相当になる
不正確さを実測)。この非対称性から、**幾何計算が要る作業はコードに、
絵の意味的な判断が要る作業はGeminiに**という役割分担を、動きの生成
そのものにまで拡張することにした。

### 新方式: メッシュ変形をやめ、Geminiが描いた複数フレームをコード側で
再生する

- 動き(表情・髪ゆれ・アクション)は、Geminiに**複数の静止フレームを
  1回のグリッド呼び出しでまとめて描かせ**、実行時はコードがそれを
  クロスフェードまたは順次再生するだけにする。ランタイムはAPI呼び出し
  無し・決定論という既定方針は維持される
- 角度・パーセンテージのような数値ラベルでフレームを個別指定するのは
  避ける(回転実験で低精度と判明済み)。**「N枚のコマで、〜が徐々に
  〜する様子を順に描く」という一括の自然文指示**にする(表情での成功、
  および回転実験での90度成功例(数値+言葉ラベル+見え方の具体描写を
  セットにしていた)から得た教訓)
- アクション等の往復運動は、**変化する方向(例: 手が上がる)だけを
  生成し、戻りはコード側でフレーム配列を逆順再生する**(ユーザー指摘、
  2026-07-24)。生成コストがほぼ半減する
- 1秒あたりに実際に描く枚数は固定12枚ではなく、**その動作の実質的な
  可動時間に応じて決める**(例: 0.4〜0.6秒の動作なら4〜5枚)。「秒間
  12フレーム」は体感の滑らかさの目標値であり、絵の枚数と一致しない
  (アニメ制作の「2コマ打ち」と同じ考え方、ユーザー指摘)
- 手だけを独立レイヤーにして動かす案は**却下**。前髪オーバーレイと同じ
  「切り分けて重ねる」構造であり、同種の破綻(継ぎ目・オクルージョン)を
  再現するリスクが高い(ユーザー指摘、2026-07-24)。フレーム密度を
  増やす方向で解決する

### スコープ縮小: 全身・360度回転・ロコモーションは対象外に

**バストアップ(胸から上、手のジェスチャーが入る余裕を持たせたやや
広めの画角)・やや左向き固定ポーズに限定する。** 全身表示・360度回転・
歩行/走行等のロコモーションは対象外とする(ユーザー決定、2026-07-24)。
理由は「動く・歪まない」の両立が最も困難だったのがまさに全身の関節
可動域だったため、可愛さ(=元絵の維持度)を優先してここを切り離した。

### コンテンツカタログと生成予算(2026-07-24時点)

512×512セル、4×4=16セル/回のグリッド詰め込みを基本単位とする
(顔クロップ相当の情報量なら512セルでも実質無劣化、本日の表情実験で
確認済み)。

| カテゴリ | 内容 | 方式 | 新規コマ数 |
|---|---|---|---|
| 表情 | core6+拡張6(neutral/happy/angry/sad/relaxed/surprised/embarrassed/troubled/smug/sleepy/crying/wink) | 静止12状態、クロスフェード切替(遷移コマ無し、コスト優先で据え置き) | 12(生成済み) |
| まばたき | 半目・閉眼 | 開いた状態(既存流用)からの往路生成+逆再生 | 2 |
| 髪ゆれ | 中央→左・中央→右(非対称キャラのためミラー不可) | 各往路4〜5コマ+逆再生でループ | 8〜10 |
| 口パク(viseme) | rest/aa/ih/ou/ee/oh | 遷移コマ無し、即時切替(ユーザー決定、2026-07-24。発話中の口は元々パタパタ切り替わるのが自然なため) | 6 |
| アクション | 頷く・首かしげ・肩すくめ・考える・手を振る・拍手など(絞り込み中) | 各往路4〜5コマ+逆再生 | 未確定(残り予算に収める) |

**合計呼び出し上限5回(ユーザー決定、2026-07-24)**。表情12は生成・
検証済み(`fixtures/gemini-verification-2026-07-24-expression/`)。
髪ゆれ+まばたき+口パクで概ね1回分(16〜18コマ)、残り4回をアクションに
充てる想定(1アクション4〜5コマなら8〜16個相当だが、種類を絞り予備を
残す方針)。

### 回転マスターの扱い(参考、対象外だが記録として残す)

本日、回転マスター2×2グリッド(30/60/90/120度)を1回試したが、90度
以外は角度精度が低く不採用(`fixtures/gemini-verification-2026-07-24-rotation/`)。
バストアップへのスコープ縮小に伴い回転マスター自体が不要になったため、
このテスト結果は「数値角度指定の精度限界」の実例記録としてのみ残す。
