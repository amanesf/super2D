...
// Lite(gemini-3.1-flash-lite-image)。studio.htmlの「1.画像投入・
...
test("Geminiモデルを選択欄で変更でき、リロード後も保持される", async () => {
  ...
  try {
    ...
    // 既定値はNano Banana 2 Lite(js/gemini_api.jsのDEFAULT_MODEL)
    const defaultValue = await page.$eval("#gemini-model-input", (e) => e.value);
    assert.equal(defaultValue, "gemini-3.1-flash-lite-image");
